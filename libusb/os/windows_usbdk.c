...
static int usbdk_get_device_list(struct libusb_context *ctx)
{
	...
	for (i = 0; i < dev_number; i++) {
		...
		if (dev == NULL) {
			...
			((struct winusb_device_priv *)usbi_get_device_priv(dev))->discovery_status = NEWLY_DISCOVERED;
			...
			((struct winusb_device_priv *)usbi_get_device_priv(dev))->discovery_status = STILL_DISCOVERED;
		}
	}
